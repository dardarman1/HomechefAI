...
def get_user_ingredients(existing_data):
    ingredients = []
...
def main():
    while True:
        action = input("Would you like to (1) create a new ingredient list, (2) view existing lists, or (3) delete a list? Type 'exit' to quit: ").strip()
        
        if action == '1':
            data = load_ingredients()
            list_name, ingredients = get_user_ingredients(data)
            if list_name and ingredients:
                save_ingredients(list_name, ingredients)
        elif action == '2':
            view_ingredient_lists()
        elif action == '3':
